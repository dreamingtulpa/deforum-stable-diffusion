    ...
    if 'google.colab' in str(ipy):
        ...
        if use_xformers_for_colab:
            ...
            all_process = [
                ['wget', x_link],
                ['pip', 'install', x_ver],
                ['ln', '-sf', 'deforum-stable-diffusion/src/ldm/modules/attention_xformers.py', 'deforum-stable-diffusion/src/ldm/modules/attention.py']
            ]
        ...
        all_process = [['ln', '-sf', 'deforum-stable-diffusion/src/ldm/modules/attention_backup.py', 'deforum-stable-diffusion/src/ldm/modules/attention.py']]
        for process in all_process:
            running = subprocess.run(process,stdout=subprocess.PIPE).stdout.decode('utf-8')
            if print_subprocess:
                print(running)
